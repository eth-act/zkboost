...
mod tests {
    ...
    fn test_signing_root_matches_lighthouse() {
        ...
        let genesis_validators_root = B256::repeat_byte(0x11);
        let object_root = B256::repeat_byte(0x22);

        let mut lighthouse_domain = [0u8; 32];
        lighthouse_domain[..4].copy_from_slice(&EXECUTION_PROOF_DOMAIN_TYPE.to_le_bytes());
        lighthouse_domain[4..].copy_from_slice(
            &ChainSpec::compute_fork_data_root(fork_version, genesis_validators_root)[..28],
        );

        let domain = execution_proof_domain(fork_version, genesis_validators_root);
        assert_eq!(domain, B256::from(lighthouse_domain));
        assert_eq!(
