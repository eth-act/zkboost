# Mock CL with zkboost

This example runs zkboost with two mock zkVM backends against a local Kurtosis testnet. `mock-cl` takes the place of the beacon node.
...
1. Run `kurtosis run --enclave local-testnet github.com/ethpandaops/ethereum-package --args-file docker/example/mock-cl/network_params.yaml`.
2. Run `docker compose -f docker/example/mock-cl/docker-compose.yml up -d`.
...
1. Run `docker compose -f docker/example/mock-cl/docker-compose.yml down`.
